...
class Player:
    def __init__(self, color_name):
        ...
        if self.color_name in ["green"]: self.image = pygame.transform.scale(self.image, (150, 100))
...
class RacerGame:
    ...
    def handle_collision(self, hit_type):
        if (self.shield or self.active_power == "repair") and hit_type in ["enemy", "barrier"]:
            self.shield = False
        ...
        if hit_type in ["enemy", "barrier"]:
            ...
            self.player.speed = max(4, self.player.speed - 10)
    ...
    def run(self):
        while not self.game_over:
            ...
            self.update()
            self.draw()
            # speed_info = font.render(f"Speed: {self.player.speed}", True, BLACK)
            # self.screen.blit(speed_info, (200, 200))
            pygame.display.flip()
            self.clock.tick(60)
        pygame.mixer.music.stop()
